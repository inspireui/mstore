...
if ( ! class_exists( '\\WC_Braintree\\Plugin_Framework\\SV_WP_Background_Job_Handler' ) ) :
...
abstract class SV_WP_Background_Job_Handler extends SV_WP_Async_Request {
	...
	public function process_job( $job, $items_per_batch = null ) {
		...
		if ( ! isset( $job->{$data_key} ) ) {
			throw new \Exception( sprintf( __( 'Job data key "%s" not set', 'woocommerce-gateway-paypal-powered-by-braintree' ), $data_key ) );
		}

		if ( ! is_array( $job->{$data_key} ) ) {
			throw new \Exception( sprintf( __( 'Job data key "%s" is not an array', 'woocommerce-gateway-paypal-powered-by-braintree' ), $data_key ) );
		}
	...
	public function add_debug_tool( $tools ) {
		...
		$tools['sv_wc_background_job_test'] = array(
			'name'     => __( 'Background Processing Test', 'woocommerce-gateway-paypal-powered-by-braintree' ),
			'button'   => __( 'Run Test', 'woocommerce-gateway-paypal-powered-by-braintree' ),
			'desc'     => __( 'This tool will test whether your server is capable of processing background jobs.', 'woocommerce-gateway-paypal-powered-by-braintree' ),
	...
	public function run_debug_tool() {
		...
		if ( $this->test_connection() ) {
			$this->debug_message = esc_html__( 'Success! You should be able to process background jobs.', 'woocommerce-gateway-paypal-powered-by-braintree' );
			$result = true;
		} else {
			$this->debug_message = esc_html__( 'Could not connect. Please ask your hosting company to ensure your server has loopback connections enabled.', 'woocommerce-gateway-paypal-powered-by-braintree' );
			$result = false;
		}
